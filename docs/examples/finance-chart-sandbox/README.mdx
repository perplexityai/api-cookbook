...
- **Use background mode for a resilient long-running task.** This script submits with `background: true` and polls the response by id, so the sandbox run can continue if the client disconnects. See [Background mode](/docs/agent-api/background-mode) for the request lifecycle.
- **The sandbox returns files.** Anything the sandbox writes to its working directory comes back as a `share_file` output item carrying a `file_id` and `filename`. Construct the `/v1/responses/{id}/files/{file_id}/content` download path from those identifiers, or list files with `GET /v1/responses/{id}/files`. The script downloads the `prices.csv` and `chart.png` artifacts directly instead of scraping anything out of `stdout`.
...
- Python 3.9+
- A Perplexity API key with Agent API access. See the [sandbox docs](https://docs.perplexity.ai/docs/agent-api/tools/sandbox) for current behavior and limits.
...
helpers. It runs a **two-phase** flow so the sandbox work survives client
disconnects while the final analysis can stream:
...
python app.py                     # serves the app on port 8000
...
payload = {
    ...
    "tools": [{"type": "sandbox"}],
    "background": True,                       # keep the long-running task alive
...
for item in body["output"]:
    if item["type"] == "share_file":
        url = f"/v1/responses/{body['id']}/files/{item['file_id']}/content"
        # item["filename"] is "prices.csv" or "chart.png"
data = get_raw(url)         # Authorization: Bearer <key>
```

The script constructs each download path from the `share_file` item's `file_id` (falling back to `GET /v1/responses/{id}/files` if none are inlined), downloads both the `.csv` and the `.png`, validates the CSV parses into ≥2 `date,close` rows, and retries the whole call if not.
...
- This example deliberately uses background mode, so callers must poll until the response reaches a terminal status
