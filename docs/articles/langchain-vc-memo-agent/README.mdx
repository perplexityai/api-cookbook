...
sidebar_position: 3
keywords: [langgraph, langchain, langsmith, agent api, web_search, finance_search, investment memo, multi-agent, citations, evaluation]
---
...
The synthesizer has no tools. It cites only from the research outputs, so anything in the memo is grounded in research the nodes actually did.
...
def build_graph():
    ...
    for section in ("team", "financials", "product", "market"):
        g.add_node(section, research_node(section))
...
Perplexity posted a perfect primary-source rate, the fastest memos, and the lowest cost per run, while tying for the best financial-concept coverage. For an output that has to withstand scrutiny — a VC memo does — that mix of source quality and speed is hard to beat.
...
└── memo/
    ...
    └── _compat.py        # temporary shim (see Limitations)
```
...
- **Client-side tool loop shim.** Section 2 drives a client-side function-tool loop to compare external search providers. The released `langchain-perplexity` (≤ 1.3.1) does not serialize `ToolMessage` / `AIMessage.tool_calls` back to the API, so `scripts/memo/_compat.py` patches the two converters. Remove it once the upstream fix ships. The core agent (Section 1) uses Perplexity's built-in server-side tools and needs no shim.
